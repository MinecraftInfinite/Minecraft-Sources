...
public class BlockChest extends BlockContainer implements IBucketPickupHandler, ILiquidContainer {
   ...
   protected BlockChest(Block.Properties properties) {
      super(properties);
      this.setDefaultState((IBlockState)((IBlockState)((IBlockState)(this.stateContainer.getBaseState()).with(FACING, EnumFacing.NORTH)).with(TYPE, ChestType.SINGLE)).with(WATERLOGGED, Boolean.valueOf(false)));
   }
   ...
   public IBlockState updatePostPlacement(IBlockState stateIn, EnumFacing facing, IBlockState facingState, IWorld worldIn, BlockPos currentPos, BlockPos facingPos) {
      ...
      if (facingState.getBlock() == this && facing.getAxis().isHorizontal()) {
         ChestType chesttype = (ChestType)facingState.get(TYPE);
         if (stateIn.get(TYPE) == ChestType.SINGLE && chesttype != ChestType.SINGLE && stateIn.get(FACING) == facingState.get(FACING) && getDirectionToAttached(facingState) == facing.getOpposite()) {
            return (IBlockState)stateIn.with(TYPE, chesttype.opposite());
         }
      } else if (getDirectionToAttached(stateIn) == facing) {
         return (IBlockState)stateIn.with(TYPE, ChestType.SINGLE);
      }
   ...
   public IBlockState getStateForPlacement(BlockItemUseContext context) {
      ...
      return (IBlockState)((IBlockState)((IBlockState)this.getDefaultState().with(FACING, enumfacing)).with(TYPE, chesttype)).with(WATERLOGGED, Boolean.valueOf(ifluidstate.getFluid() == Fluids.WATER));
   }
   ...
   private EnumFacing getDirectionToAttach(BlockItemUseContext p_196312_1_, EnumFacing p_196312_2_) {
      ...
      return iblockstate.getBlock() == this && iblockstate.get(TYPE) == ChestType.SINGLE ? (EnumFacing)iblockstate.get(FACING) : null;
   }
   ...
   public ILockableContainer getContainer(IBlockState state, World worldIn, BlockPos pos, boolean allowBlockedChest) {
      ...
      if (!(tileentity instanceof TileEntityChest)) {
         ...
         ChestType chesttype = (ChestType)state.get(TYPE);
         if (chesttype == ChestType.SINGLE) {
            ...
            if (iblockstate.getBlock() == this) {
               ChestType chesttype1 = (ChestType)iblockstate.get(TYPE);
               if (chesttype1 != ChestType.SINGLE && chesttype != chesttype1 && iblockstate.get(FACING) == state.get(FACING)) {
                  ...
                  if (tileentity1 instanceof TileEntityChest) {
                     ...
                     ilockablecontainer = new InventoryLargeChest(new TextComponentTranslation("container.chestDouble", new Object[0]), ilockablecontainer1, ilockablecontainer2);
                  }
               }
            }
   ...
   private boolean isOcelotSittingOnChest(World worldIn, BlockPos pos) {
      List<EntityOcelot> list = worldIn.<EntityOcelot>getEntitiesWithinAABB(EntityOcelot.class, new AxisAlignedBB((double)pos.getX(), (double)(pos.getY() + 1), (double)pos.getZ(), (double)(pos.getX() + 1), (double)(pos.getY() + 2), (double)(pos.getZ() + 1)));
      if (!list.isEmpty()) {
