...
public class BlockDoor extends Block {
   ...
   protected BlockDoor(Block.Properties builder) {
      super(builder);
      this.setDefaultState((IBlockState)((IBlockState)((IBlockState)((IBlockState)((IBlockState)(this.stateContainer.getBaseState()).with(FACING, EnumFacing.NORTH)).with(OPEN, Boolean.valueOf(false))).with(HINGE, DoorHingeSide.LEFT)).with(POWERED, Boolean.valueOf(false))).with(HALF, DoubleBlockHalf.LOWER));
   }
   ...
   public IBlockState updatePostPlacement(IBlockState stateIn, EnumFacing facing, IBlockState facingState, IWorld worldIn, BlockPos currentPos, BlockPos facingPos) {
      ...
      if (facing.getAxis() == EnumFacing.Axis.Y && doubleblockhalf == DoubleBlockHalf.LOWER == (facing == EnumFacing.UP)) {
         return facingState.getBlock() == this && facingState.get(HALF) != doubleblockhalf ? (IBlockState)((IBlockState)((IBlockState)((IBlockState)stateIn.with(FACING, facingState.get(FACING))).with(OPEN, facingState.get(OPEN))).with(HINGE, facingState.get(HINGE))).with(POWERED, facingState.get(POWERED)) : Blocks.AIR.getDefaultState();
      } else {
   ...
   public IBlockState getStateForPlacement(BlockItemUseContext context) {
      ...
      if (blockpos.getY() < 255 && context.getWorld().getBlockState(blockpos.up()).isReplaceable(context)) {
         ...
         return (IBlockState)((IBlockState)((IBlockState)((IBlockState)((IBlockState)this.getDefaultState().with(FACING, context.getPlacementHorizontalFacing())).with(HINGE, this.getHingeSide(context))).with(POWERED, Boolean.valueOf(flag))).with(OPEN, Boolean.valueOf(flag))).with(HALF, DoubleBlockHalf.LOWER);
      } else {
         return null;
      }
   }
   ...
   public void onBlockPlacedBy(World worldIn, BlockPos pos, IBlockState state, EntityLivingBase placer, ItemStack stack) {
      worldIn.setBlockState(pos.up(), (IBlockState)state.with(HALF, DoubleBlockHalf.UPPER), 3);
   }
   ...
   public void toggleDoor(World worldIn, BlockPos pos, boolean open) {
      ...
      if (iblockstate.getBlock() == this && iblockstate.get(OPEN) != open) {
         worldIn.setBlockState(pos, (IBlockState)iblockstate.with(OPEN, Boolean.valueOf(open)), 10);
   ...
   public IBlockState mirror(IBlockState state, Mirror mirrorIn) {
      return mirrorIn == Mirror.NONE ? state : (IBlockState)state.rotate(mirrorIn.toRotation((EnumFacing)state.get(FACING))).cycle(HINGE);
   }
