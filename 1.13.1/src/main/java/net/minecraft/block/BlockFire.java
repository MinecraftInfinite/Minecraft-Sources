...
public class BlockFire extends Block {
   ...
   private static final Map<EnumFacing, BooleanProperty> field_196449_B = (Map)BlockSixWay.FACING_TO_PROPERTY_MAP.entrySet().stream().filter((p_199776_0_) -> {
   ...
   protected BlockFire(Block.Properties builder) {
      super(builder);
      this.setDefaultState((IBlockState)((IBlockState)((IBlockState)((IBlockState)((IBlockState)((IBlockState)(this.stateContainer.getBaseState()).with(AGE, Integer.valueOf(0))).with(NORTH, Boolean.valueOf(false))).with(EAST, Boolean.valueOf(false))).with(SOUTH, Boolean.valueOf(false))).with(WEST, Boolean.valueOf(false))).with(UP, Boolean.valueOf(false)));
   }
   ...
   public IBlockState updatePostPlacement(IBlockState stateIn, EnumFacing facing, IBlockState facingState, IWorld worldIn, BlockPos currentPos, BlockPos facingPos) {
      return this.isValidPosition(stateIn, worldIn, currentPos) ? (IBlockState)this.getStateForPlacement(worldIn, currentPos).with(AGE, stateIn.get(AGE)) : Blocks.AIR.getDefaultState();
   }
      ...
      if (!iblockstate.isTopSolid() && !this.canBurn(iblockstate)) {
         ...
         for(EnumFacing enumfacing : EnumFacing.values()) {
            ...
            if (booleanproperty != null) {
               iblockstate1 = (IBlockState)iblockstate1.with(booleanproperty, Boolean.valueOf(this.canBurn(p_196448_1_.getBlockState(p_196448_2_.offset(enumfacing)))));
            }
         }
   ...
   public void tick(IBlockState state, World worldIn, BlockPos pos, Random random) {
      if (worldIn.getGameRules().getBoolean("doFireTick")) {
         ...
         if (!flag && worldIn.isRaining() && this.canDie(worldIn, pos) && random.nextFloat() < 0.2F + (float)i * 0.03F) {
            ...
            if (i != j) {
               state = (IBlockState)state.with(AGE, Integer.valueOf(j));
                     ...
                     if (l != 0 || j1 != 0 || i1 != 0) {
                        ...
                        if (l1 > 0) {
                           ...
                           if (i2 > 0 && random.nextInt(k1) <= i2 && (!worldIn.isRaining() || !this.canDie(worldIn, blockpos$mutableblockpos))) {
                              ...
                              worldIn.setBlockState(blockpos$mutableblockpos, (IBlockState)this.getStateForPlacement(worldIn, blockpos$mutableblockpos).with(AGE, Integer.valueOf(j2)), 3);
                           }
                        }
                     }
                  }
   ...
   private void catchOnFire(World worldIn, BlockPos pos, int chance, Random random, int age) {
      ...
      if (random.nextInt(chance) < i) {
         ...
         if (random.nextInt(age + 10) < 5 && !worldIn.isRainingAt(pos)) {
            ...
            worldIn.setBlockState(pos, (IBlockState)this.getStateForPlacement(worldIn, pos).with(AGE, Integer.valueOf(j)), 3);
         } else {
            worldIn.removeBlock(pos);
         }
   ...
   private boolean func_196447_a(IBlockReader p_196447_1_, BlockPos p_196447_2_) {
      for(EnumFacing enumfacing : EnumFacing.values()) {
         if (this.canBurn(p_196447_1_.getBlockState(p_196447_2_.offset(enumfacing)))) {
            return true;
         }
      }
