...
public class BlockJukebox extends BlockContainer {
   ...
   protected BlockJukebox(Block.Properties builder) {
      super(builder);
      this.setDefaultState((IBlockState)(this.stateContainer.getBaseState()).with(HAS_RECORD, Boolean.valueOf(false)));
   }

   public boolean onBlockActivated(IBlockState state, World worldIn, BlockPos pos, EntityPlayer player, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
      if (state.get(HAS_RECORD)) {
         this.func_203419_a(worldIn, pos);
         state = (IBlockState)state.with(HAS_RECORD, Boolean.valueOf(false));
   ...
   public void insertRecord(IWorld worldIn, BlockPos pos, IBlockState state, ItemStack recordStack) {
      ...
      if (tileentity instanceof TileEntityJukebox) {
         ...
         worldIn.setBlockState(pos, (IBlockState)state.with(HAS_RECORD, Boolean.valueOf(true)), 2);
      }
   }
