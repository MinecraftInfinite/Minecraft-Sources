...
public class BlockLever extends BlockHorizontalFace {
   ...
   protected BlockLever(Block.Properties builder) {
      super(builder);
      this.setDefaultState((IBlockState)((IBlockState)((IBlockState)(this.stateContainer.getBaseState()).with(HORIZONTAL_FACING, EnumFacing.NORTH)).with(POWERED, Boolean.valueOf(false))).with(FACE, AttachFace.WALL));
   }
   ...
   public VoxelShape getShape(IBlockState state, IBlockReader worldIn, BlockPos pos) {
      ...
      case FLOOR:
         switch(((EnumFacing)state.get(HORIZONTAL_FACING)).getAxis()) {
         case X:
   ...
   public boolean onBlockActivated(IBlockState state, World worldIn, BlockPos pos, EntityPlayer player, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
      state = (IBlockState)state.cycle(POWERED);
   ...
   private static void func_196379_a(IBlockState p_196379_0_, IWorld p_196379_1_, BlockPos p_196379_2_, float p_196379_3_) {
      EnumFacing enumfacing = ((EnumFacing)p_196379_0_.get(HORIZONTAL_FACING)).getOpposite();
