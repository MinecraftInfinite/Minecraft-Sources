...
public class BlockMushroom extends BlockBush implements IGrowable {
   ...
   public BlockMushroom(Block.Properties properties) {
      super(properties);
   }
   ...
   public boolean isValidPosition(IBlockState state, IWorldReaderBase worldIn, BlockPos pos) {
      ...
      if (block != Blocks.MYCELIUM && block != Blocks.PODZOL) {
         return worldIn.getLightSubtracted(pos, 0) < 13 && this.isValidGround(iblockstate, worldIn, blockpos);
      } else {
         return true;
      }
   }
   ...
   public boolean generateBigMushroom(IWorld worldIn, BlockPos pos, IBlockState state, Random rand) {
      ...
      if (feature != null && feature.place(worldIn, worldIn.getChunkProvider().getChunkGenerator(), rand, pos, IFeatureConfig.NO_FEATURE_CONFIG)) {
         return true;
      } else {
