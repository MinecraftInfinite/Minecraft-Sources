...
public class BlockPistonBase extends BlockDirectional {
   ...
   public BlockPistonBase(boolean sticky, Block.Properties properties) {
      super(properties);
      this.setDefaultState((IBlockState)((IBlockState)(this.stateContainer.getBaseState()).with(FACING, EnumFacing.NORTH)).with(EXTENDED, Boolean.valueOf(false)));
   ...
   public IBlockState getStateForPlacement(BlockItemUseContext context) {
      return (IBlockState)((IBlockState)this.getDefaultState().with(FACING, context.getNearestLookingDirection().getOpposite())).with(EXTENDED, Boolean.valueOf(false));
   }
   ...
   public boolean eventReceived(IBlockState state, World worldIn, BlockPos pos, int id, int param) {
      ...
      if (!worldIn.isRemote) {
         boolean flag = this.shouldBeExtended(worldIn, pos, enumfacing);
         if (flag && (id == 1 || id == 2)) {
            worldIn.setBlockState(pos, (IBlockState)state.with(EXTENDED, Boolean.valueOf(true)), 2);
            return false;
         }
      ...
      if (id == 0) {
         ...
         worldIn.setBlockState(pos, (IBlockState)state.with(EXTENDED, Boolean.valueOf(true)), 67);
         ...
         worldIn.setBlockState(pos, (IBlockState)((IBlockState)Blocks.MOVING_PISTON.getDefaultState().with(BlockPistonMoving.FACING, enumfacing)).with(BlockPistonMoving.TYPE, this.isSticky ? PistonType.STICKY : PistonType.DEFAULT), 3);
         worldIn.setTileEntity(pos, BlockPistonMoving.createTilePiston((IBlockState)this.getDefaultState().with(FACING, EnumFacing.byIndex(param & 7)), enumfacing, false, true));
         if (this.isSticky) {
   ...
   private boolean doMove(World worldIn, BlockPos pos, EnumFacing direction, boolean extending) {
      ...
      if (!blockpistonstructurehelper.canMove()) {
         ...
         for(int l = list.size() - 1; l >= 0; --l) {
            ...
            worldIn.setBlockState(blockpos3, (IBlockState)Blocks.MOVING_PISTON.getDefaultState().with(FACING, direction), 68);
         ...
         if (extending) {
            ...
            IBlockState iblockstate1 = (IBlockState)((IBlockState)Blocks.PISTON_HEAD.getDefaultState().with(BlockPistonExtension.FACING, direction)).with(BlockPistonExtension.TYPE, pistontype);
            IBlockState iblockstate4 = (IBlockState)((IBlockState)Blocks.MOVING_PISTON.getDefaultState().with(BlockPistonMoving.FACING, direction)).with(BlockPistonMoving.TYPE, this.isSticky ? PistonType.STICKY : PistonType.DEFAULT);
            set.remove(blockpos);
