...
public class BlockSlab extends Block implements IBucketPickupHandler, ILiquidContainer {
   ...
   public BlockSlab(Block.Properties properties) {
      super(properties);
      this.setDefaultState((IBlockState)((IBlockState)this.getDefaultState().with(TYPE, SlabType.BOTTOM)).with(WATERLOGGED, Boolean.valueOf(false)));
   }
   ...
   public IBlockState getStateForPlacement(BlockItemUseContext context) {
      ...
      if (iblockstate.getBlock() == this) {
         return (IBlockState)((IBlockState)iblockstate.with(TYPE, SlabType.DOUBLE)).with(WATERLOGGED, Boolean.valueOf(false));
      } else {
         IFluidState ifluidstate = context.getWorld().getFluidState(context.getPos());
         IBlockState iblockstate1 = (IBlockState)((IBlockState)this.getDefaultState().with(TYPE, SlabType.BOTTOM)).with(WATERLOGGED, Boolean.valueOf(ifluidstate.getFluid() == Fluids.WATER));
         EnumFacing enumfacing = context.getFace();
         return enumfacing != EnumFacing.DOWN && (enumfacing == EnumFacing.UP || !((double)context.getHitY() > 0.5D)) ? iblockstate1 : (IBlockState)iblockstate1.with(TYPE, SlabType.TOP);
      }
   }
