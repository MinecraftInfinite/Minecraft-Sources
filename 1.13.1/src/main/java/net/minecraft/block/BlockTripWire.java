...
public class BlockTripWire extends Block {
   ...
   public BlockTripWire(BlockTripWireHook p_i48305_1_, Block.Properties properties) {
      super(properties);
      this.setDefaultState((IBlockState)((IBlockState)((IBlockState)((IBlockState)((IBlockState)((IBlockState)((IBlockState)(this.stateContainer.getBaseState()).with(POWERED, Boolean.valueOf(false))).with(ATTACHED, Boolean.valueOf(false))).with(DISARMED, Boolean.valueOf(false))).with(NORTH, Boolean.valueOf(false))).with(EAST, Boolean.valueOf(false))).with(SOUTH, Boolean.valueOf(false))).with(WEST, Boolean.valueOf(false)));
   ...
   public IBlockState getStateForPlacement(BlockItemUseContext context) {
      ...
      return (IBlockState)((IBlockState)((IBlockState)((IBlockState)this.getDefaultState().with(NORTH, Boolean.valueOf(this.shouldConnectTo(iblockreader.getBlockState(blockpos.north()), EnumFacing.NORTH)))).with(EAST, Boolean.valueOf(this.shouldConnectTo(iblockreader.getBlockState(blockpos.east()), EnumFacing.EAST)))).with(SOUTH, Boolean.valueOf(this.shouldConnectTo(iblockreader.getBlockState(blockpos.south()), EnumFacing.SOUTH)))).with(WEST, Boolean.valueOf(this.shouldConnectTo(iblockreader.getBlockState(blockpos.west()), EnumFacing.WEST)));
   }

   public IBlockState updatePostPlacement(IBlockState stateIn, EnumFacing facing, IBlockState facingState, IWorld worldIn, BlockPos currentPos, BlockPos facingPos) {
      return facing.getAxis().isHorizontal() ? (IBlockState)stateIn.with(field_196537_E.get(facing), Boolean.valueOf(this.shouldConnectTo(facingState, facing))) : super.updatePostPlacement(stateIn, facing, facingState, worldIn, currentPos, facingPos);
   }
   ...
   public void onReplaced(IBlockState state, World worldIn, BlockPos pos, IBlockState newState, boolean isMoving) {
      if (!isMoving && state.getBlock() != newState.getBlock()) {
         this.notifyHook(worldIn, pos, (IBlockState)state.with(POWERED, Boolean.valueOf(true)));
      }
   }

   public void onBlockHarvested(World worldIn, BlockPos pos, IBlockState state, EntityPlayer player) {
      if (!worldIn.isRemote && !player.getHeldItemMainhand().isEmpty() && player.getHeldItemMainhand().getItem() == Items.SHEARS) {
         worldIn.setBlockState(pos, (IBlockState)state.with(DISARMED, Boolean.valueOf(true)), 4);
      }
   ...
   private void updateState(World worldIn, BlockPos pos) {
      ...
      if (flag1 != flag) {
         iblockstate = (IBlockState)iblockstate.with(POWERED, Boolean.valueOf(flag1));
