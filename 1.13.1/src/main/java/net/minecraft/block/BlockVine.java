...
public class BlockVine extends Block {
   ...
   public static final Map<EnumFacing, BooleanProperty> field_196546_A = (Map)BlockSixWay.FACING_TO_PROPERTY_MAP.entrySet().stream().filter((p_199782_0_) -> {
   ...
   public BlockVine(Block.Properties properties) {
      super(properties);
      this.setDefaultState((IBlockState)((IBlockState)((IBlockState)((IBlockState)((IBlockState)(this.stateContainer.getBaseState()).with(UP, Boolean.valueOf(false))).with(NORTH, Boolean.valueOf(false))).with(EAST, Boolean.valueOf(false))).with(SOUTH, Boolean.valueOf(false))).with(WEST, Boolean.valueOf(false)));
   }
   ...
   private IBlockState func_196545_h(IBlockState p_196545_1_, IBlockReader p_196545_2_, BlockPos p_196545_3_) {
      ...
      if (p_196545_1_.get(UP)) {
         p_196545_1_ = (IBlockState)p_196545_1_.with(UP, Boolean.valueOf(this.func_196542_b(p_196545_2_, blockpos, EnumFacing.DOWN)));
      }
      ...
      for(EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL) {
         ...
         if (p_196545_1_.get(booleanproperty)) {
            ...
            p_196545_1_ = (IBlockState)p_196545_1_.with(booleanproperty, Boolean.valueOf(flag));
         }
      }
            ...
            if (enumfacing.getAxis().isHorizontal() && !state.get(getPropertyFor(enumfacing))) {
               if (this.func_196539_a(worldIn, pos)) {
                  ...
                  if (iblockstate5.isAir()) {
                     ...
                     if (flag && this.func_196542_b(worldIn, blockpos2, enumfacing3)) {
                        worldIn.setBlockState(blockpos4, (IBlockState)this.getDefaultState().with(getPropertyFor(enumfacing3), Boolean.valueOf(true)), 2);
                     } else if (flag1 && this.func_196542_b(worldIn, blockpos3, enumfacing4)) {
                        worldIn.setBlockState(blockpos4, (IBlockState)this.getDefaultState().with(getPropertyFor(enumfacing4), Boolean.valueOf(true)), 2);
                     } else {
                        EnumFacing enumfacing1 = enumfacing.getOpposite();
                        if (flag && worldIn.isAirBlock(blockpos2) && this.func_196542_b(worldIn, pos.offset(enumfacing3), enumfacing1)) {
                           worldIn.setBlockState(blockpos2, (IBlockState)this.getDefaultState().with(getPropertyFor(enumfacing1), Boolean.valueOf(true)), 2);
                        } else if (flag1 && worldIn.isAirBlock(blockpos3) && this.func_196542_b(worldIn, pos.offset(enumfacing4), enumfacing1)) {
                           worldIn.setBlockState(blockpos3, (IBlockState)this.getDefaultState().with(getPropertyFor(enumfacing1), Boolean.valueOf(true)), 2);
                        } else if ((double)worldIn.rand.nextFloat() < 0.05D && this.func_196542_b(worldIn, blockpos4.up(), EnumFacing.UP)) {
                           worldIn.setBlockState(blockpos4, (IBlockState)this.getDefaultState().with(UP, Boolean.valueOf(true)), 2);
                        }
                     }
                  } else if (this.func_196542_b(worldIn, blockpos4, enumfacing)) {
                     worldIn.setBlockState(pos, (IBlockState)state.with(getPropertyFor(enumfacing), Boolean.valueOf(true)), 2);
                  }
               ...
               if (enumfacing == EnumFacing.UP && pos.getY() < 255) {
                  if (this.func_196541_a(worldIn, pos, enumfacing)) {
                     worldIn.setBlockState(pos, (IBlockState)state.with(UP, Boolean.valueOf(true)), 2);
                     return;
                  }
                  ...
                  if (worldIn.isAirBlock(blockpos)) {
                     ...
                     for(EnumFacing enumfacing2 : EnumFacing.Plane.HORIZONTAL) {
                        if (random.nextBoolean() || !this.func_196542_b(worldIn, blockpos.offset(enumfacing2), EnumFacing.UP)) {
                           iblockstate4 = (IBlockState)iblockstate4.with(getPropertyFor(enumfacing2), Boolean.valueOf(false));
                        }
                     }
      ...
      for(EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL) {
         if (p_196544_3_.nextBoolean()) {
            ...
            if (p_196544_1_.get(booleanproperty)) {
               p_196544_2_ = (IBlockState)p_196544_2_.with(booleanproperty, Boolean.valueOf(true));
            }
         }
      }
      ...
      for(EnumFacing enumfacing : context.getNearestLookingDirections()) {
         if (enumfacing != EnumFacing.DOWN) {
            ...
            if (!flag1 && this.func_196541_a(context.getWorld(), context.getPos(), enumfacing)) {
               return (IBlockState)iblockstate1.with(booleanproperty, Boolean.valueOf(true));
            }
         }
      }
   ...
   public IBlockState rotate(IBlockState state, Rotation rot) {
      switch(rot) {
      case CLOCKWISE_180:
         return (IBlockState)((IBlockState)((IBlockState)((IBlockState)state.with(NORTH, state.get(SOUTH))).with(EAST, state.get(WEST))).with(SOUTH, state.get(NORTH))).with(WEST, state.get(EAST));
      case COUNTERCLOCKWISE_90:
         return (IBlockState)((IBlockState)((IBlockState)((IBlockState)state.with(NORTH, state.get(EAST))).with(EAST, state.get(SOUTH))).with(SOUTH, state.get(WEST))).with(WEST, state.get(NORTH));
      case CLOCKWISE_90:
         return (IBlockState)((IBlockState)((IBlockState)((IBlockState)state.with(NORTH, state.get(WEST))).with(EAST, state.get(NORTH))).with(SOUTH, state.get(EAST))).with(WEST, state.get(SOUTH));
      default:
         return state;
      }
   }
   ...
   public IBlockState mirror(IBlockState state, Mirror mirrorIn) {
      switch(mirrorIn) {
      case LEFT_RIGHT:
         return (IBlockState)((IBlockState)state.with(NORTH, state.get(SOUTH))).with(SOUTH, state.get(NORTH));
      case FRONT_BACK:
         return (IBlockState)((IBlockState)state.with(EAST, state.get(WEST))).with(WEST, state.get(EAST));
      default:
