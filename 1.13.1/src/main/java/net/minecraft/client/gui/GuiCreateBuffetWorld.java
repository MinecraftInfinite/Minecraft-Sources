...
public class GuiCreateBuffetWorld extends GuiScreen {
   private static final List<ResourceLocation> BUFFET_GENERATORS = (List)IRegistry.CHUNK_GENERATOR_TYPE.keySet().stream().filter((p_205307_0_) -> {
      return ((ChunkGeneratorType)IRegistry.CHUNK_GENERATOR_TYPE.get(p_205307_0_)).isOptionForBuffetWorld();
   }).collect(Collectors.toList());
   private final GuiCreateWorld parent;
   private final List<ResourceLocation> biomes = Lists.<ResourceLocation>newArrayList();
   ...
   public GuiCreateBuffetWorld(GuiCreateWorld parentIn, NBTTagCompound p_i49701_2_) {
      ...
      Arrays.sort(this.biomeTypes, (p_210140_0_, p_210140_1_) -> {
         String s = ((Biome)IRegistry.BIOME.get(p_210140_0_)).getDisplayName().getString();
         String s1 = ((Biome)IRegistry.BIOME.get(p_210140_1_)).getDisplayName().getString();
   ...
   private void deserialize(NBTTagCompound p_210506_1_) {
      if (p_210506_1_.contains("chunk_generator", 10) && p_210506_1_.getCompound("chunk_generator").contains("type", 8)) {
         ...
         for(int i = 0; i < BUFFET_GENERATORS.size(); ++i) {
            if (((ResourceLocation)BUFFET_GENERATORS.get(i)).equals(resourcelocation)) {
   ...
   private NBTTagCompound serialize() {
      ...
      nbttagcompound3.putString("type", ((ResourceLocation)BUFFET_GENERATORS.get(this.field_205312_t)).toString());
   ...
   class BiomeList extends GuiSlot {
      ...
      protected void drawSlot(int slotIndex, int xPos, int yPos, int heightIn, int mouseXIn, int mouseYIn, float partialTicks) {
         this.drawString(GuiCreateBuffetWorld.this.fontRenderer, ((Biome)IRegistry.BIOME.get(GuiCreateBuffetWorld.this.biomeTypes[slotIndex])).getDisplayName().getString(), xPos + 5, yPos + 2, 16777215);
      }
   }
}
