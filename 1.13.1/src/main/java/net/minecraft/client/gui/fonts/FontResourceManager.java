...
public class FontResourceManager implements IResourceManagerReloadListener {
   ...
   private final Map<ResourceLocation, FontRenderer> fontRenderers = Maps.<ResourceLocation, FontRenderer>newHashMap();
   ...
   public void onResourceManagerReload(IResourceManager resourceManager) {
      ...
      Map<ResourceLocation, List<IGlyphProvider>> map = Maps.<ResourceLocation, List<IGlyphProvider>>newHashMap();
         ...
         try {
            for(IResource iresource : resourceManager.getAllResources(resourcelocation)) {
               try (InputStream inputstream = iresource.getInputStream()) {
                  JsonArray jsonarray = JsonUtils.getJsonArray((JsonObject)JsonUtils.fromJson(gson, IOUtils.toString(inputstream, StandardCharsets.UTF_8), JsonObject.class), "providers");
      ...
      Stream.concat(this.fontRenderers.keySet().stream(), map.keySet().stream()).distinct().forEach((p_211508_2_) -> {
         List<IGlyphProvider> list1 = map.getOrDefault(p_211508_2_, Collections.<IGlyphProvider>emptyList());
