...
public class ClientSuggestionProvider implements ISuggestionProvider {
   ...
   public CompletableFuture<Suggestions> getSuggestionsFromServer(CommandContext<ISuggestionProvider> context, SuggestionsBuilder suggestionsBuilder) {
      ...
      this.field_197018_c = new CompletableFuture<Suggestions>();
   ...
   public Collection<ISuggestionProvider.Coordinates> getCoordinates(boolean allowFloatCoords) {
      if (this.mc.objectMouseOver != null && this.mc.objectMouseOver.type == RayTraceResult.Type.BLOCK) {
         if (allowFloatCoords) {
            ...
            return Collections.<ISuggestionProvider.Coordinates>singleton(new ISuggestionProvider.Coordinates(func_209001_a(vec3d.x), func_209001_a(vec3d.y), func_209001_a(vec3d.z)));
         } else {
            BlockPos blockpos = this.mc.objectMouseOver.getBlockPos();
            return Collections.<ISuggestionProvider.Coordinates>singleton(new ISuggestionProvider.Coordinates(func_209002_a(blockpos.getX()), func_209002_a(blockpos.getY()), func_209002_a(blockpos.getZ())));
         }
      } else {
