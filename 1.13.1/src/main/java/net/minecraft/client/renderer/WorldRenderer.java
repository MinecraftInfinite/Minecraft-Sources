...
public class WorldRenderer implements IWorldEventListener, AutoCloseable, IResourceManagerReloadListener {
   ...
   private List<WorldRenderer.ContainerLocalRenderInformation> renderInfos = Lists.<WorldRenderer.ContainerLocalRenderInformation>newArrayListWithCapacity(69696);
         ...
         for(DestroyBlockProgress destroyblockprogress : this.damagedBlocks.values()) {
            ...
            if (iblockstate.getBlock().hasTileEntity()) {
               ...
               if (tileentity2 instanceof TileEntityChest && iblockstate.get(BlockChest.TYPE) == ChestType.LEFT) {
                  blockpos = blockpos.offset(((EnumFacing)iblockstate.get(BlockChest.FACING)).rotateY());
   ...
   public void setupTerrain(Entity entityIn, float partialTicks, ICamera camera, int frameCount, boolean playerSpectator) {
      ...
      if (!flag && this.displayListEntitiesDirty) {
         ...
         this.renderInfos = Lists.<WorldRenderer.ContainerLocalRenderInformation>newArrayList();
         Queue<WorldRenderer.ContainerLocalRenderInformation> queue = Queues.<WorldRenderer.ContainerLocalRenderInformation>newArrayDeque();
