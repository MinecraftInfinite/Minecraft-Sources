...
public class DebugRendererStructure implements DebugRenderer.IDebugRenderer {
   ...
   private final Map<Integer, Map<String, MutableBoundingBox>> mainBoxes = Maps.<Integer, Map<String, MutableBoundingBox>>newHashMap();
   private final Map<Integer, Map<String, MutableBoundingBox>> subBoxes = Maps.<Integer, Map<String, MutableBoundingBox>>newHashMap();
   private final Map<Integer, Map<String, Boolean>> subBoxFlags = Maps.<Integer, Map<String, Boolean>>newHashMap();
   ...
   public void render(float partialTicks, long finishTimeNano) {
      ...
      if (this.subBoxes.containsKey(i)) {
         for(Entry<String, MutableBoundingBox> entry : (this.subBoxes.get(i)).entrySet()) {
            ...
            Boolean obool = (Boolean)(this.subBoxFlags.get(i)).get(s);
   ...
   public void addStructure(MutableBoundingBox mainBox, List<MutableBoundingBox> structureSubBoxes, List<Boolean> structureSubBoxFlags, int dimensionIn) {
      if (!this.mainBoxes.containsKey(dimensionIn)) {
         this.mainBoxes.put(dimensionIn, Maps.<String, MutableBoundingBox>newHashMap());
      }

      if (!this.subBoxes.containsKey(dimensionIn)) {
         this.subBoxes.put(dimensionIn, Maps.<String, MutableBoundingBox>newHashMap());
         this.subBoxFlags.put(dimensionIn, Maps.<String, Boolean>newHashMap());
      }
