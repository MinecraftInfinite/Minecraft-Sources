...
public class MultipartBakedModel implements IBakedModel {
   ...
   private final Map<IBlockState, BitSet> field_210277_g = new Object2ObjectOpenCustomHashMap<IBlockState, BitSet>(Util.identityHashStrategy());

   public MultipartBakedModel(List<Pair<Predicate<IBlockState>, IBakedModel>> p_i48273_1_) {
      this.selectors = p_i48273_1_;
      IBakedModel ibakedmodel = (IBakedModel)(p_i48273_1_.iterator().next()).getRight();
   ...
   public List<BakedQuad> getQuads(@Nullable IBlockState state, @Nullable EnumFacing side, Random rand) {
      if (state == null) {
         ...
         for(int j = 0; j < bitset.length(); ++j) {
            if (bitset.get(j)) {
               list.addAll(((IBakedModel)((Pair)this.selectors.get(j)).getRight()).getQuads(state, side, new Random(k)));
            }
         }
   ...
   public static class Builder {
      private final List<Pair<Predicate<IBlockState>, IBakedModel>> selectors = Lists.<Pair<Predicate<IBlockState>, IBakedModel>>newArrayList();

      public void putModel(Predicate<IBlockState> predicate, IBakedModel model) {
         this.selectors.add(Pair.<Predicate<IBlockState>, IBakedModel>of(predicate, model));
      }
