...
public class VariantList implements IUnbakedModel {
   ...
   public Collection<ResourceLocation> getOverrideLocations() {
      return (Collection)this.getVariantList().stream().<ResourceLocation>map(Variant::getModelLocation).collect(Collectors.toSet());
   }

   public Collection<ResourceLocation> getTextures(Function<ResourceLocation, IUnbakedModel> modelGetter, Set<String> missingTextureErrors) {
      return (Collection)this.getVariantList().stream().<ResourceLocation>map(Variant::getModelLocation).distinct().<ResourceLocation>flatMap((p_209561_2_) -> {
