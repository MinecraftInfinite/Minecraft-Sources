...
public class Multipart implements IUnbakedModel {
   ...
   public Collection<ResourceLocation> getOverrideLocations() {
      return (Collection)this.getSelectors().stream().<ResourceLocation>flatMap((p_209563_0_) -> {
         return p_209563_0_.getVariantList().getOverrideLocations().stream();
      }).collect(Collectors.toSet());
   }

   public Collection<ResourceLocation> getTextures(Function<ResourceLocation, IUnbakedModel> modelGetter, Set<String> missingTextureErrors) {
      return (Collection)this.getSelectors().stream().<ResourceLocation>flatMap((p_209562_2_) -> {
