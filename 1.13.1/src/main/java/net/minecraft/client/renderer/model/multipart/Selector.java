   ...
   public static class Deserializer implements JsonDeserializer<Selector> {
      ...
      static ICondition getOrAndCondition(JsonObject json) {
         ...
         if (set.isEmpty()) {
            ...
            if (json.has("OR")) {
               List<ICondition> list1 = (List)Streams.stream(JsonUtils.getJsonArray(json, "OR")).<ICondition>map((p_200692_0_) -> {
                  return getOrAndCondition(p_200692_0_.getAsJsonObject());
               }).collect(Collectors.toList());
               return new OrCondition(list1);
            } else if (json.has("AND")) {
               List<ICondition> list = (List)Streams.stream(JsonUtils.getJsonArray(json, "AND")).<ICondition>map((p_200691_0_) -> {
            ...
            return new AndCondition((Iterable)set.stream().map((p_212490_0_) -> {
      ...
      private static ICondition makePropertyValue(Entry<String, JsonElement> entry) {
         return new PropertyValueCondition(entry.getKey(), ((JsonElement)entry.getValue()).getAsString());
      }
   }
}
