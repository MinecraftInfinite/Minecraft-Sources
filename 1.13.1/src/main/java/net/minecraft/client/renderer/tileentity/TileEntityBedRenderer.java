...
public class TileEntityBedRenderer extends TileEntityRenderer<TileEntityBed> {
   private static final ResourceLocation[] TEXTURES = (ResourceLocation[])Arrays.stream(EnumDyeColor.values()).sorted(Comparator.comparingInt(EnumDyeColor::getId)).map((p_199742_0_) -> {
   ...
   public void render(TileEntityBed tileEntityIn, double x, double y, double z, float partialTicks, int destroyStage) {
      ...
      if (tileEntityIn.hasWorld()) {
         ...
         this.func_199343_a(iblockstate.get(BlockBed.PART) == BedPart.HEAD, x, y, z, (EnumFacing)iblockstate.get(BlockBed.HORIZONTAL_FACING));
      } else {
