   ...
   public void render(TileEntityPiston tileEntityIn, double x, double y, double z, float partialTicks, int destroyStage) {
      ...
      if (!iblockstate.isAir() && !(tileEntityIn.getProgress(partialTicks) >= 1.0F)) {
         ...
         if (iblockstate.getBlock() == Blocks.PISTON_HEAD && tileEntityIn.getProgress(partialTicks) <= 4.0F) {
            iblockstate = (IBlockState)iblockstate.with(BlockPistonExtension.SHORT, Boolean.valueOf(true));
            ...
            IBlockState iblockstate1 = (IBlockState)((IBlockState)Blocks.PISTON_HEAD.getDefaultState().with(BlockPistonExtension.TYPE, pistontype)).with(BlockPistonExtension.FACING, iblockstate.get(BlockPistonBase.FACING));
            iblockstate1 = (IBlockState)iblockstate1.with(BlockPistonExtension.SHORT, Boolean.valueOf(tileEntityIn.getProgress(partialTicks) >= 0.5F));
            ...
            iblockstate = (IBlockState)iblockstate.with(BlockPistonBase.EXTENDED, Boolean.valueOf(true));
