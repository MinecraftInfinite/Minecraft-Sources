...
public class CommandSource implements ISuggestionProvider {
   public static final SimpleCommandExceptionType REQUIRES_PLAYER_EXCEPTION_TYPE = new SimpleCommandExceptionType(new TextComponentTranslation("permissions.requires.player", new Object[0]));
   public static final SimpleCommandExceptionType REQUIRES_ENTITY_EXCEPTION_TYPE = new SimpleCommandExceptionType(new TextComponentTranslation("permissions.requires.entity", new Object[0]));
   ...
   public CommandSource withResultConsumer(ResultConsumer<CommandSource> resultConsumerIn, BinaryOperator<ResultConsumer<CommandSource>> resultConsumerSelector) {
      ResultConsumer<CommandSource> resultconsumer = (ResultConsumer)resultConsumerSelector.apply(this.resultConsumer, resultConsumerIn);
   ...
   private void logFeedback(ITextComponent message) {
      ITextComponent itextcomponent = (new TextComponentTranslation("chat.type.admin", new Object[]{this.getDisplayName(), message})).applyTextStyles(new TextFormatting[]{TextFormatting.GRAY, TextFormatting.ITALIC});
   ...
   public Collection<ISuggestionProvider.Coordinates> getCoordinates(boolean allowFloatCoords) {
      return Collections.<ISuggestionProvider.Coordinates>singleton(ISuggestionProvider.Coordinates.DEFAULT_GLOBAL);
   }
}
