...
public class BlockPredicateArgument implements ArgumentType<BlockPredicateArgument.IResult> {
   private static final Collection<String> EXAMPLES = Arrays.<String>asList("stone", "minecraft:stone", "stone[foo=bar]", "#stone", "#stone[foo=bar]{baz=nbt}");
   private static final DynamicCommandExceptionType UNKNOWN_TAG = new DynamicCommandExceptionType((p_208682_0_) -> {
      return new TextComponentTranslation("arguments.block.tag.unknown", new Object[]{p_208682_0_});
   });
   ...
   public static Predicate<BlockWorldState> getBlockPredicate(CommandContext<CommandSource> context, String name) throws CommandSyntaxException {
      return ((BlockPredicateArgument.IResult)context.getArgument(name, BlockPredicateArgument.IResult.class)).create(((CommandSource)context.getSource()).getServer().getNetworkTagManager());
   }
