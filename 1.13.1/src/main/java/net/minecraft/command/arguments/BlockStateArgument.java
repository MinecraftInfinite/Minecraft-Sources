...
public class BlockStateArgument implements ArgumentType<BlockStateInput> {
   private static final Collection<String> EXAMPLES = Arrays.<String>asList("stone", "minecraft:stone", "stone[foo=bar]", "foo{bar=baz}");
   ...
   public static BlockStateInput getBlockState(CommandContext<CommandSource> context, String name) {
      return (BlockStateInput)context.getArgument(name, BlockStateInput.class);
   }
