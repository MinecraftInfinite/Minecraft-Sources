...
public class MessageArgument implements ArgumentType<MessageArgument.Message> {
   private static final Collection<String> EXAMPLES = Arrays.<String>asList("Hello world!", "foo", "@e", "Hello @p :)");
   ...
   public static ITextComponent getMessage(CommandContext<CommandSource> context, String name) throws CommandSyntaxException {
      return ((MessageArgument.Message)context.getArgument(name, MessageArgument.Message.class)).toComponent(context.getSource(), ((CommandSource)context.getSource()).hasPermissionLevel(2));
   }
      ...
      public static MessageArgument.Message parse(StringReader reader, boolean allowSelectors) throws CommandSyntaxException {
         ...
         if (!allowSelectors) {
            ...
            while(true) {
               ...
               while(true) {
                  if (!reader.canRead()) {
                     return new MessageArgument.Message(s, (MessageArgument.Part[])list.toArray(new MessageArgument.Part[list.size()]));
                  }
