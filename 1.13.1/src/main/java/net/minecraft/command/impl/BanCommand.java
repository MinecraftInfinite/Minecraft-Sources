...
public class BanCommand {
   ...
   public static void register(CommandDispatcher<CommandSource> dispatcher) {
      dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("ban").requires((p_198238_0_) -> {
   ...
   private static int banGameProfiles(CommandSource source, Collection<GameProfile> gameProfiles, @Nullable ITextComponent reason) throws CommandSyntaxException {
      ...
      for(GameProfile gameprofile : gameProfiles) {
         if (!userlistbans.isBanned(gameprofile)) {
            ...
            ++i;
            source.sendFeedback(new TextComponentTranslation("commands.ban.success", new Object[]{TextComponentUtils.getDisplayName(gameprofile), userlistbansentry.getBanReason()}), true);
