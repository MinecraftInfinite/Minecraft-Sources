...
public class BanIpCommand {
   ...
   private static final SimpleCommandExceptionType IP_INVALID = new SimpleCommandExceptionType(new TextComponentTranslation("commands.banip.invalid", new Object[0]));
   private static final SimpleCommandExceptionType FAILED_EXCEPTION = new SimpleCommandExceptionType(new TextComponentTranslation("commands.banip.failed", new Object[0]));

   public static void register(CommandDispatcher<CommandSource> dispatcher) {
      dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("ban-ip").requires((p_198222_0_) -> {
   ...
   private static int banIpAddress(CommandSource source, String ip, @Nullable ITextComponent reason) throws CommandSyntaxException {
      ...
      if (userlistipbans.isBanned(ip)) {
         ...
         source.sendFeedback(new TextComponentTranslation("commands.banip.success", new Object[]{ip, userlistipbansentry.getBanReason()}), true);
         if (!list.isEmpty()) {
            source.sendFeedback(new TextComponentTranslation("commands.banip.info", new Object[]{list.size(), EntitySelector.joinNames(list)}), true);
         }
