...
public class BanListCommand {
   public static void register(CommandDispatcher<CommandSource> dispatcher) {
      dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("banlist").requires((p_198233_0_) -> {
         ...
         return sendBanList((CommandSource)p_198231_0_.getSource(), Lists.newArrayList(Iterables.concat(playerlist.getBannedPlayers().getEntries(), playerlist.getBannedIPs().getEntries())));
   ...
   private static int sendBanList(CommandSource source, Collection<? extends UserListEntryBan<?>> bannedPlayerList) {
      if (bannedPlayerList.isEmpty()) {
         source.sendFeedback(new TextComponentTranslation("commands.banlist.none", new Object[0]), false);
      } else {
         source.sendFeedback(new TextComponentTranslation("commands.banlist.list", new Object[]{bannedPlayerList.size()}), false);

         for(UserListEntryBan<?> userlistentryban : bannedPlayerList) {
            source.sendFeedback(new TextComponentTranslation("commands.banlist.entry", new Object[]{userlistentryban.getDisplayName(), userlistentryban.getBannedBy(), userlistentryban.getBanReason()}), false);
         }
      }
