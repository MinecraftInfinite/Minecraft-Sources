...
public class ClearCommand {
   private static final DynamicCommandExceptionType SINGLE_FAILED_EXCEPTION = new DynamicCommandExceptionType((p_208785_0_) -> {
      return new TextComponentTranslation("clear.failed.single", new Object[]{p_208785_0_});
   });
   private static final DynamicCommandExceptionType MULTIPLE_FAILED_EXCEPTION = new DynamicCommandExceptionType((p_208787_0_) -> {
      return new TextComponentTranslation("clear.failed.multiple", new Object[]{p_208787_0_});
   });

   public static void register(CommandDispatcher<CommandSource> dispatcher) {
      dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("clear").requires((p_198247_0_) -> {
   ...
   private static int clearInventory(CommandSource source, Collection<EntityPlayerMP> targetPlayers, Predicate<ItemStack> itemPredicateIn, int maxCount) throws CommandSyntaxException {
      ...
      if (i == 0) {
         if (targetPlayers.size() == 1) {
            throw SINGLE_FAILED_EXCEPTION.create(((EntityPlayerMP)targetPlayers.iterator().next()).getName().getFormattedText());
         } else {
         ...
         if (maxCount == 0) {
            if (targetPlayers.size() == 1) {
               source.sendFeedback(new TextComponentTranslation("commands.clear.test.single", new Object[]{i, ((EntityPlayerMP)targetPlayers.iterator().next()).getDisplayName()}), true);
            } else {
               source.sendFeedback(new TextComponentTranslation("commands.clear.test.multiple", new Object[]{i, targetPlayers.size()}), true);
            }
         } else if (targetPlayers.size() == 1) {
            source.sendFeedback(new TextComponentTranslation("commands.clear.success.single", new Object[]{i, ((EntityPlayerMP)targetPlayers.iterator().next()).getDisplayName()}), true);
         } else {
            source.sendFeedback(new TextComponentTranslation("commands.clear.success.multiple", new Object[]{i, targetPlayers.size()}), true);
         }
