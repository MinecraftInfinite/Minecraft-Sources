...
public class DeOpCommand {
   private static final SimpleCommandExceptionType FAILED_EXCEPTION = new SimpleCommandExceptionType(new TextComponentTranslation("commands.deop.failed", new Object[0]));

   public static void register(CommandDispatcher<CommandSource> dispatcher) {
      dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("deop").requires((p_198325_0_) -> {
   ...
   private static int deopPlayers(CommandSource source, Collection<GameProfile> players) throws CommandSyntaxException {
      ...
      for(GameProfile gameprofile : players) {
         if (playerlist.canSendCommands(gameprofile)) {
            ...
            ++i;
            source.sendFeedback(new TextComponentTranslation("commands.deop.success", new Object[]{((GameProfile)players.iterator().next()).getName()}), true);
         }
      }
