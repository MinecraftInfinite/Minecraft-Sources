...
public class FunctionCommand {
   ...
   public static void register(CommandDispatcher<CommandSource> dispatcher) {
      dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("function").requires((p_198480_0_) -> {
   ...
   private static int executeFunctions(CommandSource source, Collection<FunctionObject> functions) {
      ...
      if (functions.size() == 1) {
         source.sendFeedback(new TextComponentTranslation("commands.function.success.single", new Object[]{i, ((FunctionObject)functions.iterator().next()).getId()}), true);
      } else {
         source.sendFeedback(new TextComponentTranslation("commands.function.success.multiple", new Object[]{i, functions.size()}), true);
      }
