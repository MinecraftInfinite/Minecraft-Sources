...
public class GameModeCommand {
   public static void register(CommandDispatcher<CommandSource> dispatcher) {
      LiteralArgumentBuilder<CommandSource> literalargumentbuilder = (LiteralArgumentBuilder)Commands.literal("gamemode").requires((p_198485_0_) -> {
         return p_198485_0_.hasPermissionLevel(2);
      });

      for(GameType gametype : GameType.values()) {
         if (gametype != GameType.NOT_SET) {
            literalargumentbuilder.then(((LiteralArgumentBuilder)Commands.literal(gametype.getName()).executes((p_198483_1_) -> {
               return setGameMode(p_198483_1_, Collections.singleton(((CommandSource)p_198483_1_.getSource()).asPlayer()), gametype);
   ...
   private static void sendGameModeFeedback(CommandSource source, EntityPlayerMP player, GameType gameTypeIn) {
      ITextComponent itextcomponent = new TextComponentTranslation("gameMode." + gameTypeIn.getName(), new Object[0]);
      if (source.getEntity() == player) {
         source.sendFeedback(new TextComponentTranslation("commands.gamemode.success.self", new Object[]{itextcomponent}), true);
      } else {
         ...
         source.sendFeedback(new TextComponentTranslation("commands.gamemode.success.other", new Object[]{player.getDisplayName(), itextcomponent}), true);
      }
