...
public class GiveCommand {
   public static void register(CommandDispatcher<CommandSource> dispatcher) {
      dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("give").requires((p_198496_0_) -> {
   ...
   private static int giveItem(CommandSource source, ItemInput itemIn, Collection<EntityPlayerMP> targets, int count) throws CommandSyntaxException {
      ...
      if (targets.size() == 1) {
         source.sendFeedback(new TextComponentTranslation("commands.give.success.single", new Object[]{count, itemIn.createStack(count, false).getTextComponent(), ((EntityPlayerMP)targets.iterator().next()).getDisplayName()}), true);
      } else {
         source.sendFeedback(new TextComponentTranslation("commands.give.success.single", new Object[]{count, itemIn.createStack(count, false).getTextComponent(), targets.size()}), true);
      }
