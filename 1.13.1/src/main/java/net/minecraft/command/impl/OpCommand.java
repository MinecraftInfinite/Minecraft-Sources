...
public class OpCommand {
   private static final SimpleCommandExceptionType ALREADY_OP = new SimpleCommandExceptionType(new TextComponentTranslation("commands.op.failed", new Object[0]));

   public static void register(CommandDispatcher<CommandSource> dispatcher) {
      dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("op").requires((p_198545_0_) -> {
   ...
   private static int opPlayers(CommandSource source, Collection<GameProfile> gameProfiles) throws CommandSyntaxException {
      ...
      for(GameProfile gameprofile : gameProfiles) {
         if (!playerlist.canSendCommands(gameprofile)) {
            ...
            ++i;
            source.sendFeedback(new TextComponentTranslation("commands.op.success", new Object[]{((GameProfile)gameProfiles.iterator().next()).getName()}), true);
         }
      }
