...
public class PardonCommand {
   ...
   public static void register(CommandDispatcher<CommandSource> dispatcher) {
      dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("pardon").requires((p_198551_0_) -> {
   ...
   private static int unbanPlayers(CommandSource source, Collection<GameProfile> gameProfiles) throws CommandSyntaxException {
      ...
      for(GameProfile gameprofile : gameProfiles) {
         if (userlistbans.isBanned(gameprofile)) {
            ...
            ++i;
            source.sendFeedback(new TextComponentTranslation("commands.pardon.success", new Object[]{TextComponentUtils.getDisplayName(gameprofile)}), true);
         }
      }
