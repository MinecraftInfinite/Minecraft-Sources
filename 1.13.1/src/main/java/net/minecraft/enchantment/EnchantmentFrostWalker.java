         ...
         for(BlockPos.MutableBlockPos blockpos$mutableblockpos1 : BlockPos.getAllInBoxMutable(pos.add((double)(-f), -1.0D, (double)(-f)), pos.add((double)f, -1.0D, (double)f))) {
            if (blockpos$mutableblockpos1.distanceSqToCenter(living.posX, living.posY, living.posZ) <= (double)(f * f)) {
               ...
               if (iblockstate1.isAir()) {
                  ...
                  if (iblockstate2.getMaterial() == Material.WATER && iblockstate2.get(BlockFlowingFluid.LEVEL) == 0 && iblockstate.isValidPosition(worldIn, blockpos$mutableblockpos1) && worldIn.checkNoEntityCollision(iblockstate, blockpos$mutableblockpos1)) {
