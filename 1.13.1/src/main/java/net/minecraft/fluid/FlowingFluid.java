...
public abstract class FlowingFluid extends Fluid {
   ...
   private static final ThreadLocal<Object2ByteLinkedOpenHashMap<Block.RenderSideCacheKey>> field_212756_e = ThreadLocal.<Object2ByteLinkedOpenHashMap<Block.RenderSideCacheKey>>withInitial(() -> {
   ...
   public IFluidState getFlowingFluidState(int level, boolean falling) {
      return (IFluidState)((IFluidState)this.getFlowingFluid().getDefaultState().with(LEVEL_1_8, Integer.valueOf(level))).with(FALLING, Boolean.valueOf(falling));
   }

   public abstract Fluid getStillFluid();

   public IFluidState getStillFluidState(boolean falling) {
      return (IFluidState)this.getStillFluid().getDefaultState().with(FALLING, Boolean.valueOf(falling));
   }
   ...
   protected Map<EnumFacing, IFluidState> func_205572_b(IWorldReaderBase p_205572_1_, BlockPos p_205572_2_, IBlockState p_205572_3_) {
      ...
      Short2ObjectMap<Pair<IBlockState, IFluidState>> short2objectmap = new Short2ObjectOpenHashMap<Pair<IBlockState, IFluidState>>();
