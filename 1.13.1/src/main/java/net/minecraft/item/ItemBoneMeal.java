...
public class ItemBoneMeal extends ItemDye {
   ...
   public EnumActionResult onItemUse(ItemUseContext context) {
      ...
      if (applyBonemeal(context.getItem(), world, blockpos)) {
         ...
         boolean flag = iblockstate.getBlockFaceShape(world, blockpos1, context.getFace()) == BlockFaceShape.SOLID;
            ...
            for(int i = 0; i < 128; ++i) {
               ...
               if (biome == Biomes.WARM_OCEAN || biome == Biomes.DEEP_WARM_OCEAN) {
                  if (i == 0 && p_203173_3_ != null && p_203173_3_.getAxis().isHorizontal()) {
                     iblockstate = (IBlockState)((Block)BlockTags.WALL_CORALS.getRandomElement(p_203173_1_.rand)).getDefaultState().with(BlockCoralWallFanDead.FACING, p_203173_3_);
                  } else if (random.nextInt(4) == 0) {
                     iblockstate = ((Block)BlockTags.UNDERWATER_BONEMEALS.getRandomElement(random)).getDefaultState();
                  }
               }

               if (iblockstate.getBlock().isIn(BlockTags.WALL_CORALS)) {
                  for(int k = 0; !iblockstate.isValidPosition(p_203173_1_, blockpos) && k < 4; ++k) {
                     iblockstate = (IBlockState)iblockstate.with(BlockCoralWallFanDead.FACING, EnumFacing.Plane.HORIZONTAL.random(random));
                  }
               }
