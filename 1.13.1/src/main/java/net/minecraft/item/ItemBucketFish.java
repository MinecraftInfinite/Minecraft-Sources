...
public class ItemBucketFish extends ItemBucket {
   ...
   private void placeFish(World p_205357_1_, ItemStack p_205357_2_, BlockPos p_205357_3_) {
      Entity entity = this.fishType.spawn(p_205357_1_, p_205357_2_, (EntityPlayer)null, p_205357_3_, true, false);
      if (entity != null) {
         ...
         if (nbttagcompound != null && nbttagcompound.contains("BucketVariantTag", 3)) {
            ...
            for(int j = 0; j < EntityTropicalFish.SPECIAL_VARIANTS.length; ++j) {
               if (i == EntityTropicalFish.SPECIAL_VARIANTS[j]) {
                  tooltip.add((new TextComponentTranslation(EntityTropicalFish.func_212324_b(j), new Object[0])).applyTextStyles(atextformatting));
                  return;
               }
            }

            tooltip.add((new TextComponentTranslation(EntityTropicalFish.func_212327_q(i), new Object[0])).applyTextStyles(atextformatting));
            ITextComponent itextcomponent = new TextComponentTranslation(s, new Object[0]);
            if (!s.equals(s1)) {
               itextcomponent.appendText(", ").appendSibling(new TextComponentTranslation(s1, new Object[0]));
            }
