...
public class ItemMap extends ItemMapBase {
   ...
   public static MapData loadMapData(IWorld worldIn, String mapId) {
      return (MapData)worldIn.getSavedData(DimensionType.OVERWORLD, MapData::new, mapId);
   }
            ...
            if ((k1 & 15) == (mapdata$mapinfo.step & 15) || flag) {
               ...
               for(int l1 = i1 - j1 - 1; l1 < i1 + j1; ++l1) {
                  if (k1 >= 0 && l1 >= -1 && k1 < 128 && l1 < 128) {
                     ...
                     int k2 = (j / i + k1 - 64) * i;
                     int l2 = (k / i + l1 - 64) * i;
                     Multiset<MaterialColor> multiset = LinkedHashMultiset.<MaterialColor>create();
   ...
   public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
      if (flagIn.isAdvanced()) {
         ...
         if (mapdata != null) {
            tooltip.add((new TextComponentTranslation("filled_map.id", new Object[]{getMapId(stack)})).applyTextStyle(TextFormatting.GRAY));
            tooltip.add((new TextComponentTranslation("filled_map.scale", new Object[]{1 << mapdata.scale})).applyTextStyle(TextFormatting.GRAY));
            tooltip.add((new TextComponentTranslation("filled_map.level", new Object[]{mapdata.scale, 4})).applyTextStyle(TextFormatting.GRAY));
         } else {
            tooltip.add((new TextComponentTranslation("filled_map.unknown", new Object[0])).applyTextStyle(TextFormatting.GRAY));
         }
      }
