...
public class NetHandlerPlayServer implements INetHandlerPlayServer, ITickable {
   ...
   public void processUpdateCommandBlock(CPacketUpdateCommandBlock packetIn) {
      ...
      if (!this.server.isCommandBlockEnabled()) {
         ...
         if (commandblockbaselogic != null) {
            EnumFacing enumfacing = (EnumFacing)this.player.world.getBlockState(blockpos).get(BlockCommandBlock.FACING);
            switch(packetIn.getMode()) {
            case SEQUENCE:
               IBlockState iblockstate1 = Blocks.CHAIN_COMMAND_BLOCK.getDefaultState();
               this.player.world.setBlockState(blockpos, (IBlockState)((IBlockState)iblockstate1.with(BlockCommandBlock.FACING, enumfacing)).with(BlockCommandBlock.CONDITIONAL, Boolean.valueOf(packetIn.isConditional())), 2);
               break;
            case AUTO:
               IBlockState iblockstate = Blocks.REPEATING_COMMAND_BLOCK.getDefaultState();
               this.player.world.setBlockState(blockpos, (IBlockState)((IBlockState)iblockstate.with(BlockCommandBlock.FACING, enumfacing)).with(BlockCommandBlock.CONDITIONAL, Boolean.valueOf(packetIn.isConditional())), 2);
               break;
            case REDSTONE:
            default:
               IBlockState iblockstate2 = Blocks.COMMAND_BLOCK.getDefaultState();
               this.player.world.setBlockState(blockpos, (IBlockState)((IBlockState)iblockstate2.with(BlockCommandBlock.FACING, enumfacing)).with(BlockCommandBlock.CONDITIONAL, Boolean.valueOf(packetIn.isConditional())), 2);
            }
            ...
            if (!StringUtils.isNullOrEmpty(s)) {
               this.player.sendMessage(new TextComponentTranslation("advMode.setCommand.success", new Object[]{s}));
            }
         }
   ...
   public void processUpdateCommandMinecart(CPacketUpdateCommandMinecart packetIn) {
      ...
      if (!this.server.isCommandBlockEnabled()) {
         ...
         if (commandblockbaselogic != null) {
            ...
            commandblockbaselogic.updateCommand();
            this.player.sendMessage(new TextComponentTranslation("advMode.setCommand.success", new Object[]{packetIn.getCommand()}));
         }
            ...
            if (tileentitystructure.hasName()) {
               ...
               if (packetIn.func_210384_b() == TileEntityStructure.UpdateCommand.SAVE_AREA) {
                  if (tileentitystructure.save()) {
                     this.player.sendStatusMessage(new TextComponentTranslation("structure_block.save_success", new Object[]{s}), false);
                  } else {
                     this.player.sendStatusMessage(new TextComponentTranslation("structure_block.save_failure", new Object[]{s}), false);
                  }
               } else if (packetIn.func_210384_b() == TileEntityStructure.UpdateCommand.LOAD_AREA) {
                  if (!tileentitystructure.isStructureLoadable()) {
                     this.player.sendStatusMessage(new TextComponentTranslation("structure_block.load_not_found", new Object[]{s}), false);
                  } else if (tileentitystructure.load()) {
                     this.player.sendStatusMessage(new TextComponentTranslation("structure_block.load_success", new Object[]{s}), false);
                  } else {
                     this.player.sendStatusMessage(new TextComponentTranslation("structure_block.load_prepare", new Object[]{s}), false);
                  }
               } else if (packetIn.func_210384_b() == TileEntityStructure.UpdateCommand.SCAN_AREA) {
                  if (tileentitystructure.detectSize()) {
                     this.player.sendStatusMessage(new TextComponentTranslation("structure_block.size_success", new Object[]{s}), false);
                  } else {
                     this.player.sendStatusMessage(new TextComponentTranslation("structure_block.size_failure", new Object[0]), false);
                  }
               }
            } else {
               this.player.sendStatusMessage(new TextComponentTranslation("structure_block.invalid_structure_name", new Object[]{packetIn.getName()}), false);
            }
   ...
   public void processTryUseItemOnBlock(CPacketPlayerTryUseItemOnBlock packetIn) {
      ...
      if (blockpos.getY() < this.server.getBuildLimit() - 1 || enumfacing != EnumFacing.UP && blockpos.getY() < this.server.getBuildLimit()) {
         ...
         ITextComponent itextcomponent = (new TextComponentTranslation("build.tooHigh", new Object[]{this.server.getBuildLimit()})).applyTextStyle(TextFormatting.RED);
   ...
   public void onDisconnect(ITextComponent reason) {
      ...
      this.server.getPlayerList().sendMessage((new TextComponentTranslation("multiplayer.player.left", new Object[]{this.player.getDisplayName()})).applyTextStyle(TextFormatting.YELLOW));
   ...
   public void processChatMessage(CPacketChatMessage packetIn) {
      ...
      if (this.player.getChatVisibility() == EntityPlayer.EnumChatVisibility.HIDDEN) {
         this.sendPacket(new SPacketChat((new TextComponentTranslation("chat.cannotSend", new Object[0])).applyTextStyle(TextFormatting.RED)));
      } else {
         this.player.markPlayerActive();
