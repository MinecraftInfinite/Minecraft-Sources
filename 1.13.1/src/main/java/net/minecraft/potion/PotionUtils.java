...
public class PotionUtils {
   ...
   public static void addPotionTooltip(ItemStack itemIn, List<ITextComponent> lores, float durationFactor) {
      ...
      if (list.isEmpty()) {
         lores.add((new TextComponentTranslation("effect.none", new Object[0])).applyTextStyle(TextFormatting.GRAY));
      } else {
         for(PotionEffect potioneffect : list) {
            ITextComponent itextcomponent = new TextComponentTranslation(potioneffect.getEffectName(), new Object[0]);
            ...
            if (potioneffect.getAmplifier() > 0) {
               itextcomponent.appendText(" ").appendSibling(new TextComponentTranslation("potion.potency." + potioneffect.getAmplifier(), new Object[0]));
            }
      ...
      if (!list1.isEmpty()) {
         ...
         lores.add((new TextComponentTranslation("potion.whenDrank", new Object[0])).applyTextStyle(TextFormatting.DARK_PURPLE));
         ...
         for(Tuple<String, AttributeModifier> tuple : list1) {
            ...
            if (d0 > 0.0D) {
               lores.add((new TextComponentTranslation("attribute.modifier.plus." + attributemodifier2.getOperation(), new Object[]{ItemStack.DECIMALFORMAT.format(d1), new TextComponentTranslation("attribute.name." + (String)tuple.getA(), new Object[0])})).applyTextStyle(TextFormatting.BLUE));
            } else if (d0 < 0.0D) {
               d1 = d1 * -1.0D;
               lores.add((new TextComponentTranslation("attribute.modifier.take." + attributemodifier2.getOperation(), new Object[]{ItemStack.DECIMALFORMAT.format(d1), new TextComponentTranslation("attribute.name." + (String)tuple.getA(), new Object[0])})).applyTextStyle(TextFormatting.RED));
            }
         }
      }
