...
public abstract class MinecraftServer implements IThreadListener, ISnooperInfo, ICommandSource, Runnable {
   ...
   private final Map<DimensionType, WorldServer> worlds = Maps.<DimensionType, WorldServer>newIdentityHashMap();
   ...
   protected final Map<DimensionType, long[]> timeOfLastDimensionTick = Maps.<DimensionType, long[]>newIdentityHashMap();
   ...
   private final ResourcePackList<ResourcePackInfo> resourcePacks = new ResourcePackList<ResourcePackInfo>(ResourcePackInfo::new);
   ...
   public void convertMapIfNeeded(String worldNameIn) {
      if (this.getActiveAnvilConverter().isOldMapFormat(worldNameIn)) {
         ...
         this.setUserMessage(new TextComponentTranslation("menu.convertingLevel", new Object[0]));
   ...
   public void loadAllWorlds(String saveName, String worldNameIn, long seed, WorldType type, JsonElement generatorOptions) {
      ...
      this.setUserMessage(new TextComponentTranslation("menu.loadingLevel", new Object[0]));
   ...
   public void loadDataPacks(File p_195560_1_, WorldInfo p_195560_2_) {
      ...
      List<ResourcePackInfo> list = Lists.<ResourcePackInfo>newArrayList();
   ...
   public void initialWorldChunkLoad(WorldSavedDataStorage p_71222_1_) {
      ...
      int l = 192;
      int i1 = 625;
      this.setUserMessage(new TextComponentTranslation("menu.generatingTerrain", new Object[0]));
      WorldServer worldserver = this.getWorld(DimensionType.OVERWORLD);
      ...
      for(int j1 = -192; j1 <= 192 && this.isServerRunning(); j1 += 16) {
         ...
         while(!completablefuture.isDone()) {
            try {
               ...
               this.setCurrentTaskAndPercentDone(new TextComponentTranslation("menu.preparingSpawn", new Object[0]), set.size() * 100 / 625);
            }
         }
      ...
      for(DimensionType dimensiontype : DimensionType.func_212681_b()) {
         ForcedChunksSaveData forcedchunkssavedata = (ForcedChunksSaveData)p_71222_1_.get(dimensiontype, ForcedChunksSaveData::new, "chunks");
         if (forcedchunkssavedata != null) {
            WorldServer worldserver1 = this.getWorld(dimensiontype);
            LongIterator longiterator = forcedchunkssavedata.getChunks().iterator();

            while(longiterator.hasNext()) {
               this.setCurrentTaskAndPercentDone(new TextComponentTranslation("menu.loadingForcedChunks", new Object[]{dimensiontype}), forcedchunkssavedata.getChunks().size() * 100 / 625);
   ...
   public void run() {
      try {
         if (this.init()) {
            ...
            this.statusResponse.setVersion(new ServerStatusResponse.Version("1.13.1", 401));
   ...
   public void updateTimeLightAndEntities(BooleanSupplier p_71190_1_) {
      ...
      for(WorldServer worldserver : this.getWorlds()) {
         ...
         ((long[])this.timeOfLastDimensionTick.computeIfAbsent(worldserver.dimension.getType(), (p_212377_0_) -> {
            return new long[100];
      ...
      for(int j = 0; j < this.tickables.size(); ++j) {
         ((ITickable)this.tickables.get(j)).tick();
      }
   ...
   private void loadDataPacks(WorldInfo worldInfoIn) {
      ...
      this.resourcePacks.setEnabledPacks(list);
      List<IResourcePack> list1 = Lists.<IResourcePack>newArrayList();
         ...
         if (userlistwhitelist.isLanServer()) {
            for(EntityPlayerMP entityplayermp : Lists.newArrayList(playerlist.getPlayers())) {
               if (!userlistwhitelist.isWhitelisted(entityplayermp.getGameProfile())) {
                  entityplayermp.connection.disconnect(new TextComponentTranslation("multiplayer.disconnect.not_whitelisted", new Object[0]));
               }
            }
   ...
   public int getPermissionLevel(GameProfile profile) {
      if (this.getPlayerList().canSendCommands(profile)) {
         UserListOpsEntry userlistopsentry = (UserListOpsEntry)this.getPlayerList().getOppedPlayers().getEntry(profile);
