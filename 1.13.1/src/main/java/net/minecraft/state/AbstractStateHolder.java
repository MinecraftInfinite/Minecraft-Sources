...
public abstract class AbstractStateHolder<O, S> implements IStateHolder<S> {
   ...
   public <T extends Comparable<T>> S cycle(IProperty<T> property) {
      return (S)this.with(property, cyclePropertyValue(property.getAllowedValues(), this.get(property)));
   }
   ...
   public String toString() {
      ...
      if (!this.getValues().isEmpty()) {
         ...
         stringbuilder.append((String)this.getValues().entrySet().stream().map(MAP_ENTRY_TO_STRING).collect(Collectors.joining(",")));
         stringbuilder.append(']');
      }
   ...
   public void buildPropertyValueTable(Map<Map<IProperty<?>, Comparable<?>>, S> map) {
      if (this.propertyToStateMap != null) {
         ...
         Table<IProperty<?>, Comparable<?>, S> table = HashBasedTable.<IProperty<?>, Comparable<?>, S>create();
