...
public class EnumProperty<T extends Enum<T> & IStringSerializable> extends AbstractProperty<T> {
   ...
   public Optional<T> parseValue(String value) {
      return Optional.<T>ofNullable(this.nameToValue.get(value));
   }
   ...
   public static <T extends Enum<T> & IStringSerializable> EnumProperty<T> create(String name, Class<T> clazz, Predicate<T> filter) {
      return create(name, clazz, (Collection)Arrays.stream(clazz.getEnumConstants()).filter(filter).collect(Collectors.toList()));
   }
