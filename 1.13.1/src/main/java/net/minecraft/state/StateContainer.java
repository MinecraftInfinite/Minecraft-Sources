...
public class StateContainer<O, S extends IStateHolder<S>> {
   ...
   protected <A extends AbstractStateHolder<O, S>> StateContainer(O object, StateContainer.IFactory<O, S, A> factory, Map<String, IProperty<?>> propertiesIn) {
      ...
      Map<Map<IProperty<?>, Comparable<?>>, A> map = Maps.<Map<IProperty<?>, Comparable<?>>, A>newLinkedHashMap();
      ...
      for(IProperty<?> iproperty : this.properties.values()) {
         stream = stream.<List<Comparable<?>>>flatMap((p_200999_1_) -> {
      ...
      stream.forEach((p_201000_5_) -> {
         Map<IProperty<?>, Comparable<?>> map1 = MapPopulator.<IProperty<?>, Comparable<?>>createMap(this.properties.values(), p_201000_5_);
   ...
   public static class Builder<O, S extends IStateHolder<S>> {
      private final O owner;
      private final Map<String, IProperty<?>> properties = Maps.<String, IProperty<?>>newHashMap();
      ...
      public <A extends AbstractStateHolder<O, S>> StateContainer<O, S> create(StateContainer.IFactory<O, S, A> factory) {
         return new StateContainer<O, S>(this.owner, factory, this.properties);
      }
   }
