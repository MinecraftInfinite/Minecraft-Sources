...
public class Stat<T> extends ScoreCriteria {
   ...
   protected Stat(StatType<T> typeIn, T valueIn, IStatFormater formatterIn) {
   ...
   public static <T> String buildName(StatType<T> typeIn, T valueIn) {
      return locationToKey(IRegistry.STATS.getKey(typeIn)) + ":" + locationToKey(typeIn.getRegistry().getKey(valueIn));
   }
