...
public class TagCollection<T> {
   ...
   private final Map<ResourceLocation, Tag<T>> tagMap = Maps.<ResourceLocation, Tag<T>>newHashMap();
   ...
   public Collection<ResourceLocation> getOwningTags(T itemIn) {
      ...
      for(Entry<ResourceLocation, Tag<T>> entry : this.tagMap.entrySet()) {
         if (((Tag)entry.getValue()).contains(itemIn)) {
            ...
            for(IResource iresource : resourceManager.getAllResources(resourcelocation)) {
               try {
                  ...
                  if (jsonobject == null) {
                     ...
                     Tag.Builder<T> builder = map.getOrDefault(resourcelocation1, Tag.Builder.<T>create());
