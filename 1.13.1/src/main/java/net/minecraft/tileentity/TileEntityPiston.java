...
public class TileEntityPiston extends TileEntity implements ITickable {
   ...
   private IBlockState getCollisionRelatedBlockState() {
      return !this.isExtending() && this.shouldPistonHeadBeRendered() ? (IBlockState)((IBlockState)Blocks.PISTON_HEAD.getDefaultState().with(BlockPistonExtension.TYPE, this.pistonState.getBlock() == Blocks.STICKY_PISTON ? PistonType.STICKY : PistonType.DEFAULT)).with(BlockPistonExtension.FACING, this.pistonState.get(BlockPistonBase.FACING)) : this.pistonState;
   }
         ...
         if (this.pistonState != null && this.world.getBlockState(this.pos).getBlock() == Blocks.MOVING_PISTON) {
            ...
            if (iblockstate.isAir()) {
               ...
               if (iblockstate.has(BlockStateProperties.WATERLOGGED) && iblockstate.get(BlockStateProperties.WATERLOGGED)) {
                  iblockstate = (IBlockState)iblockstate.with(BlockStateProperties.WATERLOGGED, Boolean.valueOf(false));
               }
   ...
   public VoxelShape getCollisionShape(IBlockReader p_195508_1_, BlockPos p_195508_2_) {
      ...
      if (!this.extending && this.shouldHeadBeRendered) {
         voxelshape = ((IBlockState)this.pistonState.with(BlockPistonBase.EXTENDED, Boolean.valueOf(true))).getCollisionShape(p_195508_1_, p_195508_2_);
      } else {
      ...
      if ((double)this.progress < 1.0D && enumfacing == this.getMotionDirection()) {
         ...
         if (this.shouldPistonHeadBeRendered()) {
            iblockstate = (IBlockState)((IBlockState)Blocks.PISTON_HEAD.getDefaultState().with(BlockPistonExtension.FACING, this.pistonFacing)).with(BlockPistonExtension.SHORT, Boolean.valueOf(this.extending != 1.0F - this.progress < 4.0F));
         } else {
