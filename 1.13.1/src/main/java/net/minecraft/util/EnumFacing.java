...
public enum EnumFacing implements IStringSerializable {
   ...
   private static final Map<String, EnumFacing> NAME_LOOKUP = (Map)Arrays.stream(VALUES).collect(Collectors.toMap(EnumFacing::getName2, (p_199787_0_) -> {
      return p_199787_0_;
   }));
   private static final EnumFacing[] BY_INDEX = (EnumFacing[])Arrays.stream(VALUES).sorted(Comparator.comparingInt((p_199790_0_) -> {
      return p_199790_0_.index;
   })).toArray((p_199788_0_) -> {
      return new EnumFacing[p_199788_0_];
   });
   private static final EnumFacing[] BY_HORIZONTAL_INDEX = (EnumFacing[])Arrays.stream(VALUES).filter((p_199786_0_) -> {
   ...
   public static enum Axis implements Predicate<EnumFacing>, IStringSerializable {
      ...
      private static final Map<String, EnumFacing.Axis> NAME_LOOKUP = (Map)Arrays.stream(values()).collect(Collectors.toMap(EnumFacing.Axis::getName2, (p_199785_0_) -> {
         return p_199785_0_;
      }));
      private final String name;
   ...
   public static enum Plane implements Iterable<EnumFacing>, Predicate<EnumFacing> {
      ...
      public Iterator<EnumFacing> iterator() {
         return Iterators.<EnumFacing>forArray(this.facingValues);
      }
   }
}
