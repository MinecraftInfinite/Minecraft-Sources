...
public abstract class Scheduler<K, T extends ITaskType<K, T>, R> {
   ...
   private final List<CompletableFuture<R>> unbatchedFutures = Lists.<CompletableFuture<R>>newArrayList();
   ...
   public CompletableFuture<R> schedule(K key) {
      ...
      CompletableFuture<CompletableFuture<R>> completablefuture1 = CompletableFuture.<CompletableFuture<R>>supplyAsync(supplier, this.schedulerPool);
   ...
   public CompletableFuture<R> gather() {
      ...
      CompletableFuture<R> completablefuture1 = CompletableFuture.allOf((CompletableFuture[])this.unbatchedFutures.toArray(new CompletableFuture[0])).thenCompose((p_202850_1_) -> {
   ...
   public final class FutureWrapper {
      ...
      public FutureWrapper(K keyIn, R resultIn, T taskType) {
         ...
         for(this.result = resultIn; taskType != null; taskType = (T)taskType.getPreviousTaskType()) {
            this.futures.put(taskType, CompletableFuture.<R>completedFuture(resultIn));
         }
      ...
      private CompletableFuture<R> getFuture(CompletableFuture<R> existingFuture, T taskType) {
         Map<K, CompletableFuture<R>> map = new ConcurrentHashMap<K, CompletableFuture<R>>();
         return this.futures.computeIfAbsent(taskType, (p_202915_4_) -> {
            if (taskType.getPreviousTaskType() == null) {
               ...
               taskType.acceptInRange(this.key, (p_202918_3_, p_202918_4_) -> {
                  CompletableFuture completablefuture2 = map.put((K)p_202918_3_, Scheduler.this.onTaskStart(p_202918_3_, Scheduler.this.createFutureWrapper(p_202918_3_)).getFuture(existingFuture, p_202918_4_));
               });
               CompletableFuture<?>[] completablefuture = (CompletableFuture[])Streams.concat(Stream.of(existingFuture), map.values().stream()).toArray((p_202916_0_) -> {
