...
public class BlockStateFlattenGenOptions extends DataFix {
   ...
   private Dynamic<?> fix(Dynamic<?> p_209636_1_) {
      return p_209636_1_.getString("generatorName").equalsIgnoreCase("flat") ? p_209636_1_.update("generatorOptions", (p_209634_1_) -> {
         return (Dynamic)DataFixUtils.orElse(p_209634_1_.getStringValue().map(this::fixString).map(p_209634_1_::createString), p_209634_1_);
      }) : p_209636_1_;
   }
   ...
   String fixString(String p_199180_1_) {
      if (p_199180_1_.isEmpty()) {
         ...
         if (i >= 0 && i <= 3) {
            ...
            stringbuilder.append((String)StreamSupport.stream(field_199182_b.split(s1).spliterator(), false).map((p_206368_2_) -> {
