...
public class ChunkPaletteFormat extends DataFix {
   ...
   public static String getProperty(Dynamic<?> p_209719_0_, String p_209719_1_) {
      return (String)p_209719_0_.get("Properties").map((p_209710_1_) -> {
   ...
   static class Section {
      private final IntIdentityHashBiMap<Dynamic<?>> palette = new IntIdentityHashBiMap<Dynamic<?>>(32);
      ...
      public int y;
      private final Set<Dynamic<?>> seen = Sets.<Dynamic<?>>newIdentityHashSet();
      ...
      public int upgrade(int p_199207_1_) {
         if (!this.hasData) {
            ...
            ChunkPaletteFormat.NibbleArray chunkpaletteformat$nibblearray = (ChunkPaletteFormat.NibbleArray)this.section.get("Data").flatMap(Dynamic::getByteBuffer).map((p_210055_0_) -> {
               return new ChunkPaletteFormat.NibbleArray(DataFixUtils.toArray(p_210055_0_));
            }).orElseGet(ChunkPaletteFormat.NibbleArray::new);
            ChunkPaletteFormat.NibbleArray chunkpaletteformat$nibblearray1 = (ChunkPaletteFormat.NibbleArray)this.section.get("Add").flatMap(Dynamic::getByteBuffer).map((p_210052_0_) -> {
   ...
   static final class UpgradeChunk {
      ...
      private final int x;
      private final int z;
      private final Int2ObjectMap<Dynamic<?>> tileEntities = new Int2ObjectLinkedOpenHashMap<Dynamic<?>>(16);
