...
public class EntityArmorAndHeld extends DataFix {
   ...
   private <IS> TypeRewriteRule cap(Type<IS> p_206323_1_) {
      ...
      OpticFinder<Pair<Either<List<IS>, Unit>, Dynamic<?>>> opticfinder = DSL.<Pair<Either<List<IS>, Unit>, Dynamic<?>>>typeFinder(type);
      OpticFinder<List<IS>> opticfinder1 = DSL.<List<IS>>fieldFinder("Equipment", DSL.list(p_206323_1_));
      return this.fixTypeEverywhereTyped("EntityEquipmentToArmorAndHandFix", this.getInputSchema().getType(TypeReferences.ENTITY), this.getOutputSchema().getType(TypeReferences.ENTITY), (p_207448_4_) -> {
         Either<List<IS>, Unit> either = Either.<List<IS>, Unit>right(DSL.unit());
         Either<List<IS>, Unit> either1 = Either.<List<IS>, Unit>right(DSL.unit());
         ...
         if (optional.isPresent()) {
            ...
            if (!list.isEmpty()) {
               either = Either.<List<IS>, Unit>left(Lists.newArrayList(list.get(0), is));
            }
            ...
            if (list.size() > 1) {
               ...
               either1 = Either.<List<IS>, Unit>left(list1);
            }
         }
