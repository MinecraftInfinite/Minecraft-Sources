...
public class ServerTickList<T> implements ITickList<T> {
   ...
   protected final Set<NextTickListEntry<T>> pendingTickListEntriesHashSet = Sets.<NextTickListEntry<T>>newHashSet();
   protected final TreeSet<NextTickListEntry<T>> pendingTickListEntriesTreeSet = new TreeSet<NextTickListEntry<T>>();
   private final WorldServer world;
   private final List<NextTickListEntry<T>> pendingTickListEntriesThisTick = Lists.<NextTickListEntry<T>>newArrayList();
      ...
      for(int i = 0; i < 2; ++i) {
         ...
         while(iterator.hasNext()) {
            ...
            if (blockpos.getX() >= p_205366_1_.minX && blockpos.getX() < p_205366_1_.maxX && blockpos.getZ() >= p_205366_1_.minZ && blockpos.getZ() < p_205366_1_.maxZ) {
               ...
               if (list == null) {
                  list = Lists.<NextTickListEntry<T>>newArrayList();
               }
   ...
   public NBTTagList write(Chunk p_205363_1_) {
      ...
      for(NextTickListEntry<T> nextticklistentry : list) {
         ...
         nbttagcompound.putString("i", ((ResourceLocation)this.serializer.apply(nextticklistentry.getTarget())).toString());
   ...
   private void addEntry(BlockPos p_205370_1_, T p_205370_2_, int p_205370_3_, TickPriority p_205370_4_) {
      NextTickListEntry<T> nextticklistentry = new NextTickListEntry<T>(p_205370_1_, p_205370_2_, (long)p_205370_3_ + this.world.getGameTime(), p_205370_4_);
