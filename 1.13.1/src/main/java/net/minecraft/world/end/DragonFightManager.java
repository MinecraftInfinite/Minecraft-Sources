...
public class DragonFightManager {
   ...
   private final BossInfoServer bossInfo = (BossInfoServer)(new BossInfoServer(new TextComponentTranslation("entity.minecraft.ender_dragon", new Object[0]), BossInfo.Color.PINK, BossInfo.Overlay.PROGRESS)).setPlayEndBossMusic(true).setCreateFog(true);
   ...
   public DragonFightManager(WorldServer worldIn, NBTTagCompound compound) {
      ...
      if (compound.contains("Gateways", 9)) {
         ...
         this.gateways.addAll(ContiguousSet.<Integer>create(Range.closedOpen(0, 20), DiscreteDomain.integers()));
   ...
   private void func_210827_g() {
      ...
      List<EntityDragon> list = this.world.<EntityDragon>getEntities(EntityDragon.class, EntitySelectors.IS_ALIVE);
      if (list.isEmpty()) {
         this.dragonKilled = true;
      } else {
   ...
   private void func_210828_h() {
      ...
      if (list.isEmpty()) {
         ...
         this.dragonUniqueId = ((EntityDragon)list.get(0)).getUniqueID();
      }
   ...
   public void tryRespawnDragon() {
      if (this.dragonKilled && this.respawnState == null) {
         ...
         for(EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL) {
            List<EntityEnderCrystal> list = this.world.<EntityEnderCrystal>getEntitiesWithinAABB(EntityEnderCrystal.class, new AxisAlignedBB(blockpos1.offset(enumfacing, 2)));
            if (list.isEmpty()) {
               return;
            }
