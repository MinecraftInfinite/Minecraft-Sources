...
public class ChunkGeneratorFlat extends AbstractChunkGenerator<FlatGenSettings> {
   ...
   private Biome func_202099_e() {
         ...
         if (compositefeature != null) {
            for(CompositeFeature<?, ?> compositefeature1 : compositefeature) {
               ...
               if (feature instanceof Structure) {
                  ...
                  chunkgeneratorflat$biomewrapper.addStructure((Structure)feature, ifeatureconfig != null ? ifeatureconfig : (IFeatureConfig)FlatGenSettings.field_202249_l.get(compositefeature1));
               }
            }
         }
      }
      ...
      if (flag) {
         List<GenerationStage.Decoration> list = Lists.<GenerationStage.Decoration>newArrayList();
