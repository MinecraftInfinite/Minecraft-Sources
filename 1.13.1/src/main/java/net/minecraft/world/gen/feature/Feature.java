...
public abstract class Feature<C extends IFeatureConfig> {
   private static final List<Biome.SpawnListEntry> EMPTY_SPAWN_LIST = Lists.<Biome.SpawnListEntry>newArrayList();
   ...
   public static final Map<String, Structure<?>> STRUCTURES = (Map)Util.make(Maps.newHashMap(), (p_205170_0_) -> {
   ...
   public static boolean isPositionInStructureExact(IWorld worldIn, String structureIn, BlockPos pos) {
      return ((Structure)STRUCTURES.get(structureIn.toLowerCase(Locale.ROOT))).isPositionInsideStructure(worldIn, pos);
   }
}
