      ...
      if (!this.func_203427_a(worldIn, position, i)) {
         ...
         for(int j = 0; j < i; ++j) {
            ...
            if (iblockstate.isAir() || iblockstate.isIn(BlockTags.LEAVES)) {
               this.func_208520_a(changedBlocks, worldIn, position.up(j), this.trunk);
            }

            if (j < i - 1) {
               iblockstate = worldIn.getBlockState(position.add(1, j, 0));
               if (iblockstate.isAir() || iblockstate.isIn(BlockTags.LEAVES)) {
                  this.func_208520_a(changedBlocks, worldIn, position.add(1, j, 0), this.trunk);
               }

               iblockstate = worldIn.getBlockState(position.add(1, j, 1));
               if (iblockstate.isAir() || iblockstate.isIn(BlockTags.LEAVES)) {
                  this.func_208520_a(changedBlocks, worldIn, position.add(1, j, 1), this.trunk);
               }

               iblockstate = worldIn.getBlockState(position.add(0, j, 1));
               if (iblockstate.isAir() || iblockstate.isIn(BlockTags.LEAVES)) {
                  this.func_208520_a(changedBlocks, worldIn, position.add(0, j, 1), this.trunk);
               }
            }
         }
