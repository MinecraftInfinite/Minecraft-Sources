...
public class TreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
   ...
   private void placeCocoa(IWorld worldIn, int p_181652_2_, BlockPos pos, EnumFacing side) {
      this.setBlockState(worldIn, pos, (IBlockState)((IBlockState)Blocks.COCOA.getDefaultState().with(BlockCocoa.AGE, Integer.valueOf(p_181652_2_))).with(BlockCocoa.HORIZONTAL_FACING, side));
   }

   private void addVine(IWorld worldIn, BlockPos pos, BooleanProperty prop) {
      this.setBlockState(worldIn, pos, (IBlockState)Blocks.VINE.getDefaultState().with(prop, Boolean.valueOf(true)));
   }
